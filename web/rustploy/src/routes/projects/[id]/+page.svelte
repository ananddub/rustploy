<script lang="ts">
	...
	import { RocketIcon, FolderOpen, Box, Layers2, Plus, Settings2 } from '@lucide/svelte';
...
<PageLayout>
		...
		<div class="flex items-center gap-2">
			...
			<CreateServiceDropdown
				onSelect={(type) => {
					...
					if (type === 'compose') showCreateCompose = true;
				}}
			/>
		</div>
	</header>
	...
	<main class="flex-1 px-8 pb-8">
		...
		{:else if apps.length === 0 && composes.length === 0}
			...
			<div class="flex flex-wrap gap-3">
				...
				{#each composes as compose (compose.id)}
					<div
						...
						onkeydown={(e) =>
							e.key === 'Enter' && goto(`/projects/${projectId}/compose/${compose.id}`)}
					>
						<div class="flex items-center gap-3">
							<div
								class="w-9 h-9 rounded-lg bg-secondary/10 flex items-center justify-center shrink-0 relative"
							>
								<Layers2 class="w-4 h-4 text-secondary" />
								<span
									class="absolute -top-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border-2 border-card {appStatusDot(compose.compose_status ?? '')}"
								></span>
							</div>
							<div class="min-w-0">
								<p
									class="font-medium text-sm truncate group-hover:text-secondary transition-colors"
								>
									{compose.name}
								</p>
						...
						<div class="flex items-center justify-between border-t border-border pt-2">
							<span class="flex items-center gap-1.5 text-xs text-muted-foreground">
								<span
									class="w-1.5 h-1.5 rounded-full {appStatusDot(compose.compose_status ?? '')}"
								></span>
...
{#if showCreateCompose && selectedEnvId}
	<CreateComposeModal
		...
		onCreated={(compose) => {
			goto(`/projects/${projectId}/compose/${compose.id}`);
		}}
	/>
{/if}
