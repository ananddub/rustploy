<script lang="ts">
	...
	import { composeControllerPatch } from '$lib/client/sdk.gen';
	import type { ComposeResponseDto } from '$lib/client/types.gen';
	import loader from '@monaco-editor/loader';

	type Props = { compose: ComposeResponseDto; onUpdated: (c: ComposeResponseDto) => void; active: boolean };
	let { compose, onUpdated, active }: Props = $props();

	let envVars = $state(compose.env_var ?? '');
	...
	$effect(() => { if (active && editorInstance) setTimeout(() => editorInstance.layout(), 10); });

	async function save() {
		saving = true;
		try {
			const res = await composeControllerPatch({ path: { id: compose.id }, body: { env_var: envVars } });
...
<div class="bg-card border border-border rounded-lg p-6 flex flex-col gap-4">
	<div>
		...
		<p class="text-sm text-muted-foreground mt-1">Set environment variables in <code class="font-mono">KEY=VALUE</code> format.</p>
	</div>
