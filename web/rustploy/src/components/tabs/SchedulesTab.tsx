...
type Props = {
  serviceLabel?: string;
  /** Passed through to future API call: POST /schedules { application_id } or { compose_id } */
  serviceId: number;
};
...
/**
 * SchedulesTab — shared between Application and Compose pages.
 */
export default function SchedulesTab(props: Props) {
  ...
  // TODO: replace with createResource once GET/POST /schedules is available
      ...
      <section class="bg-base-200 border border-base-300 rounded-lg p-6">
        ...
        <div class="flex flex-col gap-4">
          ...
          <div class="flex justify-end">
            ...
            >
              {saving() ? <span class="loading loading-spinner loading-xs" /> : <Plus class="w-4 h-4" />}
              Add Schedule
            </button>
          </div>
        </div>
            ...
            {(s) => (
              <div class="grid grid-cols-[1fr_160px_140px_140px_80px] gap-4 items-center px-5 py-3 border-b border-base-300 last:border-0 hover:bg-base-300/40 transition-colors">
                ...
                <div class="text-xs text-base-content/40 flex items-center gap-1">
                  <Clock class="w-3 h-3" />{s.last_run ? formatDate(s.last_run) : '—'}
                </div>
                <div class="text-xs text-base-content/40 flex items-center gap-1">
                  <Play class="w-3 h-3" />{s.next_run ? formatDate(s.next_run) : '—'}
                </div>
                <div class="flex justify-end">
