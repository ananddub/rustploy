...
export default function EnvironmentTab(props: Props) {
  const [envVars, setEnvVars] = createSignal(props.app.env_var ?? '');
  ...
  // When tab becomes visible, trigger layout so Monaco renders properly
  createEffect(() => {
    if (props.active && editorInstance) {
    ...
    <div class="bg-base-200 border border-base-300 rounded-lg p-6 flex flex-col gap-4">
      <div>
        ...
        <p class="text-sm text-base-content/40 mt-1">
          Set environment variables. One per line in <code class="font-mono">KEY=VALUE</code> format.
        </p>
      </div>
