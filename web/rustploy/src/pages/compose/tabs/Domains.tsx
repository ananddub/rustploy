...
import type { ComposeResponseDto, DomainResponseDto } from '../../../client/types.gen';
import { domainControllerListByCompose, domainControllerDelete } from '../../../client/sdk.gen';
import { formatDate } from '../../../lib/utils';
import AddDomainModal from '../../applications/tabs/domains/AddDomainModal';
import EditDomainModal from '../../applications/tabs/domains/EditDomainModal';
...
export default function ComposeDomainsTab(props: Props) {
  ...
  const [domains, { mutate, refetch }] = createResource(
    () => props.compose.id,
    async (composeId) => {
      const res = await domainControllerListByCompose({ path: { compose_id: composeId } });
      return res.data ?? [];
    },
  );
  ...
  return (
    <div class="flex flex-col gap-6">
      ...
      <section class="bg-base-200 border border-base-300 rounded-lg p-6">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-sm text-base-content/40 mt-1">Configure custom domains and routing for this compose service.</p>
          </div>
      ...
      <section class="bg-base-200 border border-base-300 rounded-lg overflow-hidden">
        ...
        <Show when={!domains.loading && (domains() ?? []).length === 0}>
          <div class="flex flex-col items-center justify-center py-16 text-base-content/30">
            ...
            <p class="text-xs mt-1">Add a domain above to expose this compose service.</p>
          </div>
        </Show>
      ...
      {/* AddDomainModal passes compose_id via appId — backend accepts compose_id on CreateDomainDto */}
      <Show when={showAdd()}>
        <AddDomainModal
          appId={props.compose.id}
          composeId={props.compose.id}
