...
impl DatabaseService {
    pub(super) async fn select_mariadb(&self, id: i64) -> sqlx::Result<DatabaseRecord> {
        sqlx::query_as!(
            DatabaseRecord,
            r#"SELECT 'mariadb' AS "kind: DatabaseKind", id AS "id!: i64", name, app_name, description, docker_image,
    ...
    pub(super) async fn create_mariadb(
        &self,
    ...
    pub(super) async fn patch_mariadb(
        &self,
        id: i64,
    ...
    pub(super) async fn delete_mariadb(&self, id: i64) -> sqlx::Result<()> {
    ...
    pub(super) async fn get_mariadb_server_id_and_name(&self, id: i64) -> sqlx::Result<(Option<i64>, String)> {
        let r = sqlx::query!("SELECT server_id, name FROM mariadb_dbs WHERE id = ?", id)
            .fetch_one(self.db.as_ref())
            .await?;
