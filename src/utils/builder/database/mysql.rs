...
use crate::repository::MysqlRepository;
...
pub async fn build_mysql_stack(
    db_id: i64,
    mounts: &[Mount],
) -> ExecResult<(String, String, String)> {
    let repo = auto_di::resolve::<MysqlRepository>().await.map_err(|e| ExecError::CommandFailed {
        code: None,
        stderr: format!("Failed to resolve MysqlRepository: {}", e),
    })?;
    let db = repo.get_details(db_id).await.map_err(|e| ExecError::CommandFailed {
        code: None,
        stderr: format!("Failed to fetch mysql db: {}", e),
    })?;
    ...
    resolved_env.insert("MYSQL_DATABASE".to_string(), db.database_name.clone());
    resolved_env.insert("MYSQL_USER".to_string(), db.database_user.clone());
    resolved_env.insert("MYSQL_PASSWORD".to_string(), db.database_password.clone());
    resolved_env.insert("MYSQL_ROOT_PASSWORD".to_string(), db.database_root_password.clone());
    ...
    let yaml = serde_yaml::to_string(&file).map_err(|e| ExecError::CommandFailed {
        code: None,
        stderr: format!("Failed to generate mysql yaml: {}", e),
    })?;
